...
public final class FindMeetingQueryTest {
  ...
  private static final int TIME_1200AM = TimeRange.getTimeInMinutes(0, 0);
  ...
  private static final int DURATION_HALF_DAY = 720;
  ...
  @Test
  public void everyOptionalAttendeeIsConsideredButIgnored() {
    // Have each person have different events. We should see three options because each person has
    // split the restricted times and the optional person gets ignored.
    //
    // Events  :       |--A--|     |--B--|
    // Events  : |--------------C--------------|
    // Day     : |-----------------------------|
  ...
  @Test
  public void onlyOptionalAttendeesWithoutFreeTime() {
    // Have each person have different events. We should see two options because each person has
    // split the restricted times.
    //
    // Events  : |-----A-------||------B-------|
    ...
    Collection<Event> events = Arrays.asList(
        new Event("Event 1", TimeRange.fromStartDuration(TIME_1200AM, DURATION_HALF_DAY),
            Arrays.asList(PERSON_A)),
        new Event("Event 2", TimeRange.fromStartDuration(TIME_1200PM, DURATION_HALF_DAY),
            Arrays.asList(PERSON_B)));
  ...
  public void justEnoughRoomWithoutOptional() {
    // Have two person, but make it so that there is not enough room at one point in the day to
    // have the meeting with the optional person. Therefore, this person gets ignored.
    //
    // Events  : |--A--|     |----A----|
    // Events  :       |-B-|
    // Day     : |---------------------|
